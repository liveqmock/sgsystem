	...
	<form id="pagerForm" onsubmit="return divSearch(this, 'user_lookup_box');" action="user/lookUp.do" method="get">
		...
		<div class="searchBar">
			<table class="searchContent" style="margin-left: 20px">
				<tr >
					<td >
						姓名：<input type="text" size="10" name="trueName" value="<c:out value="${param.trueName }" />"/>
					</td>
		...
		<tbody>
			<c:forEach var="u" items="${requestScope.users }" >
				...
				<tr>
					...
					<td title="${u.trueName }"><c:out value="${u.trueName }" /></td>
					<td>
						<c:out value="${u.number }" />
					</td>
					<td ><input name="ids"  type="checkbox" value="{id:'${u.id }', zh:'${u.userName }',name:'${u.trueName }',number:${u.number }}"/></td>
