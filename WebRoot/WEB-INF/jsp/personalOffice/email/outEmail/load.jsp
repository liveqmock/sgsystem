...
<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);" action="email/loadOut.do" method="get">
		<div class="searchBar">
			...
			<div class="searchBar">
				<table class="searchContent" style="margin-left:50px">
					<tr >
						...
						<td>
							标题：<input type="text" name="name" style="width: 90px" value="<c:out value="${param.name }" />" />
						</td>
						<td>发送日期：
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<my:validationAuthority url="email/delOut.do">
				<li><a class="delete" href="email/delOut.do" target="selectedTodo" rel="ids" title="确定要删除吗?" warn="至少选择一条信息"><span>批量删除</span></a></li>
	...
	<table class="table" width="1100" layoutH="114">
		...
		<tbody>
			<c:forEach var="v" items="${values }" >
				...
				<tr target="id" rel="${v.emOutbox.id }">
				
					<td title="id：${v.emOutbox.id }"><input name="ids" value="${v.emOutbox.id }" type="checkbox"></td>
					<td title="${v.emOutbox.eoName }">
					<a href="email/findOut.do?id=${v.emOutbox.id}" target="navTab" rel="${param.rel}_find">
					<c:out value="${v.emOutbox.eoName}" />
					</a>
					</td>
					<td title="${v.emOutbox.eoUid}"><my:outTrueName id="${v.emOutbox.eoUid }"/></td>
					

					
					<td title="<fmt:formatDate value="${v.emOutbox.eoDate}" pattern="yyyy-MM-dd HH:mm:ss" />"><fmt:formatDate value="${v.emOutbox.eoDate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
