...
<form method="post" action="email/addDraft.do" enctype="multipart/form-data" class="pageForm required-validate" onsubmit="return iframeCallback(this,navTabAjaxDone)">
	...
	<div class="pageFormContent nowrap"   layoutH="56">
			<dl>
				...
				<dd>
					<a href="user/lookUpPage.do?type=2" lookupGroup="user" title="用户查询" >
						<input type="text" readonly="readonly" class="clear_user required"  toName="user.name"  size="80" value="<my:outTrueName id="${inUid}"/> "/>
					</a>
					<input type="hidden"   name="userIds" toName="user.id" value="${inUid }" class="clear_user" />
			...
			<dl>
				<dt>标题：</dt>
				<dd >
					<input type="text" name="eoName" class="required"  maxlength="50" value="<c:out value="${email.emOutbox.eoName }"/>" class="required"  size="80" />
				</dd>
			</dl>
			...
			<dl>
				<dt>内容：</dt>
				<dd>
				<textarea  name="eoDetail" cols="100" rows="15" class="editor" ><c:out value="${email.emOutbox.eoDetail }" /></textarea>
				</dd>
			</dl>
			<input type="hidden" name="type" value="out"/>
			<input type="hidden" name="id" value="${email.emOutbox.id}" />
