...
function validateCallback(form, callback, confirmMsg) {
	...
	var _submitFn = function(){
		//提交表单是将提交按钮设为不可提交
		//$("button:submit",$form).attr({"disabled":"disabled"}).parent().parent().attr("class","buttonDisabled");
		$.ajax({
			...
			success:function(data){
				//将提交按钮设为可点
				$("button:submit",$(this)).removeAttr("disabled").parent().parent().attr("class","buttonActive");
				//判断服务器是否返回错误
				if(data!=null){
					callback(data) || DWZ.ajaxDone(data);
				}else{
					if (alertMsg) {
						alertMsg.error(DWZ.msg("parseError")); //操作失败，解析异常
					} else {
						alert("操作失败！服务器错误！");//操作失败，解析异常
					}
				}
			},
...
function iframeCallback(form, callback){
	...
	_iframeResponse($form,$iframe[0], callback || DWZ.ajaxDone);
...
function _iframeResponse($form,iframe, callback){
	...
	$iframe.bind("load", function(event){
		...
		//恢复提交按钮的状态
		//$("button:submit",$form).removeAttr("disabled").parent().parent().attr("class","buttonActive");
		//判断服务器是否返回错误
		if(response!=null){
			callback(response);
		}else{
