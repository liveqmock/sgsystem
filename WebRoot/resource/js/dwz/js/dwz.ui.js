	...
	*/
	if ($.fn.uploadify) {
		$(":file[uploaderOption]", $p).each(function(){
			var $this = $(this);
			var options = {
				swf: "resource/js/uploadify/scripts/uploadify.swf",//swf路径.
				fileObjName: $this.attr("name") || "file",//修改上传文件的接收名称
				auto: true,//是否自动上传
				...
				onUploadSuccess: function(file,data,response) {//上传完成时触发（每个文件触发一次）
					alert( 'id: ' + file.id+ ' - 索引: ' + file.index
						...
						+ ' - 服务器端消息: ' + data
						+ ' - 是否上传成功: ' + response);
						/**var d=DWZ.jsonEval(data);
						
						if(d.mes=="s"){
							$(".uploadfilename",$p).html(file.name+"&nbsp;&nbsp;&nbsp;&nbsp;已上传完成");
							$(".filepath",$p).val(d.savepath);
							$(".filename",$p).val(file.name);
						}else{
							$(".uploadfilename",$p).html(file.name+"&nbsp;&nbsp;&nbsp;&nbsp;上传失败");
						}
						$("button[type='submit']",$p).removeAttr("disabled").parent().parent().attr("class","buttonActive");*/
					}
			};
	...
	// 这里放其他第三方jQuery插件...
	//鼠标拖拉改变元素大小
	if ($.fn.myreSize) $("[reStyleSize]", $p).myreSize();
	...
	if (UE.ui.Editor) {
		$("textarea.editor", $p).each(function(){
			...
			options.initialFrameWidth=$this.attr("width")||850;
	...
	//清空指定文本域，resetClass=class名称
	if($.fn.formReset)$("a[resetClass]", $p).delText();
	
	// 字典值查询，生成列表
	
	if ($.fn.showSelectList) $("select[showSelectList]", $p).showSelectList();
	
	$("dl dt",$p).css("text-align","right");
}
