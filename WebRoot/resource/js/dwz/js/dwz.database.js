...
(function($){
	...
	$.extend({
		...
		//以追加形式
		appendBringBackSuggest: function(args){
			var $box = _lookup['$target'].parents(".unitBox:first");
			$box.find(":input").each(function(){
				//修改name属性为toName 避免冲突
				var $input = $(this), inputName = $input.attr("toName");
				for (var key in args) {
					var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
					if (name == inputName) {
						...
						if(oldval.length==0){
							...
							if(newvalue.length!=0){
								$input.val(oldval+","+newvalue.join(","));
							}
						}
						break;
					}
	...
	$.fn.extend({
		lookup: function(){
			return this.each(function(){
				var $this = $(this), options = {mask:true, 
					width:$this.attr('width')||1000, height:$this.attr('height')||400,
		...
		//多选并追加
		appendMultLookup: function(){
			return this.each(function(){
				var $this = $(this), args={};
				$this.click(function(event){
					var $unitBox = $this.parents(".unitBox:first");
					$unitBox.find("[name='"+$this.attr("appendMultLookup")+"']").filter(":checked").each(function(){
						var _args = DWZ.jsonEval($(this).val());
						for (var key in _args) {
							var value = args[key] ? args[key]+"," : "";
							args[key] = value + _args[key];
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'lookup':
						...
						//修改此处
						html = '<input type="hidden" name="'+field.name+'"  toName="'+field.lookupGroup+'.'+field.lookupPk+'"/>'
							+ '<input type="text" toName="'+field.toName+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'" readonly="readonly" />'
